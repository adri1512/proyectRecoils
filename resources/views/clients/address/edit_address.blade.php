...
@section('client_content')
  <div class="d-flex align-items-center mb-3">
    ...
    <h1 class="fw-bold mb-0">Editar Dirección</h1>
  </div>
  <p class="mb-4">Modifica los datos de tu dirección de recolección</p>

  <form class="ps-5 pe-5" id="form_address" method="POST" action="{{ route('client_address_update', $address->id) }}">
    @csrf
    @method('PUT')
    <!-- NOMBRE DE LA DIRECCIÓN -->
    <div class="mb-3 position-relative">
      <label for="name_address" class="form-label">Nombre dirección</label>
      <input type="text" class="form-control" id="name_address" name="name_address" value="{{ ucfirst($address->name) }}" maxlength="25" title="Has alcanzado el máximo de 25 caracteres" required>
    </div>
    ...
    <div class="row g-3 mb-3">
      ...
      <div class="col-md-6">
        ...
        <select class="form-select" id="department_address" name="department_address" required>
          ...
          @foreach($departments as $d)
            <option value="{{ $d->id }}" {{ $d->id == $address->town->id_department ? 'selected' : '' }}>{{ $d->name }}</option>
          @endforeach
        </select>
      </div>
      ...
      <div class="col-md-6">
        ...
        <select class="form-select" id="town_address" name="town_address" required>
          ...
          @foreach($towns as $t)
            @if($t->id_department == $address->town->id_department)
              <option value="{{ $t->id }}" {{ $t->id == $address->id_town ? 'selected' : '' }}>{{ $t->name }}</option>
            @endif
          @endforeach
        </select>
      </div>
    ...
    <div class="row g-3 mb-3">
      ...
      <div class="col-md-6">
        ...
        <input type="text" class="form-control" id="neighborhood_address" name="neighborhood_address" value="{{ ucfirst($address->neighborhood) }}" maxlength="25" title="Has alcanzado el máximo de 25 caracteres">
      </div>

      <!-- DIRECCIÓN -->
      <div class="col-md-6">
        <label for="street_address" class="form-label">Dirección</label>
        <input type="text" class="form-control" id="street_address" name="street_address" value="{{ $address->address }}" maxlength="80" title="Has alcanzado el máximo de 80 caracteres" required>
      </div>
    </div>
    ...
    <div class="mb-3">
      ...
      <textarea class="form-control" id="reference_address" name="reference_address" maxlength="100" rows="2" title="Has alcanzado el máximo de 100 caracteres">{{ ucfirst($address->reference) }}</textarea>
    </div>

    <!-- BOTON DE ACTUALIZAR DIRECCIÓN -->
    <div class="text-center mt-4 mb-3">
      <button type="submit" class="btn btn_green">
        <i class="fa-regular fa-paper-plane me-2"></i> Actualizar Dirección
      </button>
    </div>
  </form>
@endsection
  ...
  <script>
    document.addEventListener('DOMContentLoaded', function () {
      ...
      document.getElementById('department_address').addEventListener('change', function() {
        ...
        let selectTown = document.getElementById('town_address');
